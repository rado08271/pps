...
cl_event createEvent(cl_context context) {
    ...
    size_t size = 0;
    clGetEventInfo(event, CL_EVENT_COMMAND_EXECUTION_STATUS, 0, NULL, &size);
    cl_int info = 0;
    clGetEventInfo(event, CL_EVENT_COMMAND_EXECUTION_STATUS, size, &info, NULL);
    printf("%d\n", info);

    if (err != CL_SUCCESS) {
        printf("\nCL Program status reading failed with %d\n", err);
        return NULL;
    }
...
size_t getWorkerGroupSize(uul invokes) {
    for (size_t i = 1 ; ; i*=2) {
        if (invokes < i) {
            printf("\nClosest to %llu is %zu", invokes, i);
            return i;
        }
    }
